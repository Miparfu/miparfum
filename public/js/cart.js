(function(){
  ...
  app.loadResources = function(callback){
    ...
    requireJQuery(baseUrl + '/vendor/jquery-1.10.2.js', fork(callback, function(jQuery){
      ...
      loadCss('css/bootstrap-widgets.css'
      , 'bootstrap-widget-loaded', fork(callback, function(){
        loadCss('css/cart.css', 'cart-loaded', done)
      }))
      loadJs(baseUrl + '/bootstrap.js', done)
      loadJs(baseUrl + '/salejs/languages/' + language + '.js', done)
    }))
  }
  ...
  app.initialize = function(options, callback){
    ...
    // Options.
    this.baseUrl  = options.baseUrl  || 'https://селективная-парфюмерия-оптом.рф/libs'
    //this.baseUrl  = options.baseUrl  || 'https://vorlis.ru/Miparfum'
  ...
  app.initializeViews = function(){
    ...
    app.on('send order', bind(function(){
      if(app.contacts.isValid()){
        ...
        location="https://селективная-парфюмерия-оптом.рф/zakaz";
        // Sending order to server.
        server.post('/orders.php', order, bind(function(err){
          if(err){
    ...
    $(document).on('click', '.cart-buy-button', bind(function(e){
      ...
      var carty = $('.cart-button').offset();
      $('.animtocart').animate({ top: carty.top + 'px', left: carty.left + 'px', width: 0, height: 0 }, 800, function(){
        $(this).remove();
      });
  ...
  app.template('cart-purchase-button', function(add, totalPrice){
    add('<span class="cart__btn">Купить парфюм</span>')
  ...
  app.template('cart-item', function(add, item){
    add('<div class="cart-item" data-name="' + escapeHtml(item.name) + '">')
      ...
      + '">&times;</a>')
      add('<input class="cart-item-quantity form-control" type="number" value="'
  ...
  app.ContactsView = function(contacts, cart){
    ...
    this.$el = $('<div class="cart" style="width: 100%;height: 300px;"></div>')
    ...
    var sendOrder = bind(function(e){
      ...
      ym(52373854,'reachGoal','SendOrder');
