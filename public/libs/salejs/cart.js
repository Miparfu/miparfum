(function(){
  ...
  app.loadResources = function(callback){
    ...
    requireJQuery(baseUrl + '/vendor/jquery-1.10.2.js', fork(callback, function(jQuery){
      ...
      loadCss(baseUrl + '/libs/bootstrap/css/bootstrap-widget.css'
      , 'bootstrap-widget-loaded', fork(callback, function(){
        loadCss(baseUrl + '/css/cart.css', 'cart-loaded', done)
      }))

      loadJs(baseUrl + '/libs/bootstrap/js/bootstrap.js', done)
      loadJs(baseUrl + '/libs/salejs/languages/' + language + '.js', done)
    }))
  }
  ...
  app.initialize = function(options, callback){
    ...
    // Options.
    this.baseUrl  = options.baseUrl  || 'http://miparfum.ru'
    ...
    if(!this.initialized){
      ...
      app.off()
      $(document).off('click', '.product-cart')
  ...
  app.initializeViews = function(){
    ...
    app.on('send order', bind(function(){
      if(app.contacts.isValid()){
        ...
        server.post(this.baseUrl + '/orders.php', order, bind(function(err){
          if(err){
    ...
    $(document).on('click', '.product-cart', bind(function(e){
      e.preventDefault()
  ...
  proto.validate = function(){
    ...
    // }
   // console.info.apply(console, this.phone)
